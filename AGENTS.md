...
Before tagging: add a `## X.Y.Z` section to `CHANGELOG.md` and run `python3 scripts/build-changelog.py` (regenerates the site changelog page; the push gate rejects a release tag without its section, and any push with a stale `docs/changelog.html`). The tag's Release workflow then deploys everything: the note-sharing Worker (`web-share/`, migrations applied first, before the app builds), the DMG + appcast, and the site to the `main` production branch. Manual fallbacks if a step fails: `cd web-share && npm run deploy` and `npx -y wrangler pages deploy --branch main`.
